module aptos_friend_addr::aptos_friend {
    ...
    public entry fun buy_share(
        ...
        if (exists<Holding>(holding_obj_addr)) {
            ...
            vector::push_back(
                &mut issuer.holder_holdings, get_holding_obj(issuer_addr, sender_addr)
            );
    ...
    public fun get_holding_obj_addr(
        issuer_addr: address, holder_addr: address
    ): address {
    ...
    #[view]
    /// Get issuer
    public fun get_issuer(issuer_obj: Object<Issuer>): (address, String, u64) acquires Issuer {
    ...
    public fun calculate_buy_share_cost(
        issuer_obj: Object<Issuer>, amount: u64
    ...
    fun construct_issuer_object_seed(issuer_addr: address): vector<u8> {
        bcs::to_bytes(
            &string_utils::format2(&b"{}_issuer_{}", @aptos_friend_addr, issuer_addr)
        )
    }
    ...
    fun construct_user_object_seed(user_addr: address): vector<u8> {
        bcs::to_bytes(
            &string_utils::format2(&b"{}_user_{}", @aptos_friend_addr, user_addr)
        )
    }
